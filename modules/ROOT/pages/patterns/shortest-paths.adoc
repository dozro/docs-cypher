...
=== Use an index to guarantee SSP(All)

// integrate the following admonition and expand
...
// add index for planner to prove that endpoint cardinality is 1

// note that SHORTEST 1 for simple patterns won't get rewritten to legacy shortest if it has an undirected relationship pattern  and a lower bound of 1 or more

// note: GPM shortest + single rel. pattern, undirected and lower bound of reps > 0 -> normally rewritten to legacy shortest, but in this case, much worse performance (why?)